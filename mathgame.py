...
class game: 
	...
	'''
        ...
        if self.L:
            for g in self.L[:-1]:
                builder += (g.name + ", ")
            builder += (self.L[-1].name)
        builder += "|"
        if self.R:
            for g in self.R[:-1]:
                builder += (g.name + ", ")
            builder += (self.R[-1].name)
        builder += "}"
        return builder
    ...
    def isnumber(self):
	'''
	Returns true if your game is a surreal number
	'''
        for g in self.L:
            for h in self.R:
                if h <= g:
	...
	'''
        if self.isnumber():
	    #NOTE Hey this uses list methods which are depreciated in terms
	    #     of using the set type. Until that's fixed this does nothing.
	    #self.L.sort()
            #self.R.sort()
            #del self.L[:-1]
            #del self.R[1:]
